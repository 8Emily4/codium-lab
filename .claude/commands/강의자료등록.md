...
#### 유료/무료 · 가격 확인 (등록 전 필수 질문)

본문이 준비되면 **등록 직전에 반드시** 아래 두 가지를 사용자에게 물어보세요(이미 명시했으면 생략):

1. **무료 / 유료?**
   - **무료** → `access: "public"` — 로그인한 사용자 누구나 전체 열람.
   - **유료** → `access: "restricted"` — 접근권한을 받은 사용자만 전체 열람. 권한 없는 사람에겐 제목 + 본문 일부(미리보기)만 노출됩니다.
2. **유료라면 가격(원)?** → `price` 에 정수로(예: `50000`). 무료면 가격은 불필요(저장 시 자동 무시).

> 가격은 숫자(원)만. 무료(`public`)로 등록하면 `price` 는 무시되어 비워집니다.
> 개별 사용자에게 기간을 지정해 권한을 주는 것은 앱 [자료 관리 → 접근권한] 에서 합니다.
...
| `access` |  | `public`(무료·로그인 전체) / `restricted`(유료·권한자만, 기본) |
| `price` |  | 유료 자료의 가격(원, 정수). 무료면 생략(자동 무시) |
| `category` |  | 분류 (예: "영상 제작") |
| `tags` |  | string 배열 |
...
node -e '
  ...
  process.stdout.write(JSON.stringify({
    ...
    access:   "restricted",      // public(무료) | restricted(유료)
    price:    50000,             // 유료일 때 가격(원). 무료면 생략
    category: "<분류>",
    tags:     ["태그1","태그2"],
    authorName:"코디움랩",
    // 수정 시: id:"<uuid>"
...
(예: "등록 완료 — 제목 / 유료 50,000원·published / id=…" 또는 "… / 무료·published / id=…").
본문에 이미지를 추가했다면
