...
import { faqPage } from "@/lib/brand";

export default function FaqPage() {
  const [activeIdx, setActiveIdx] = useState(0);

  const flat = useMemo(
    () => faqPage.categories.flatMap((c) => c.items.map((i) => ({ ...i, cat: c.label }))),
    [],
  );

  const active = faqPage.categories[activeIdx];
            ...
            <Reveal>
              <aside className="lg:sticky lg:top-24">
                ...
                <p className="mt-8 text-xs text-zinc-500 dark:text-zinc-400">
                  전체 {flat.length}개 질문 · 카테고리별로 정리되어 있습니다.
                </p>
              </aside>
            </Reveal>
