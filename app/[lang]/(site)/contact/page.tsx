...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: dict.contact.metaTitle, description: dict.contact.metaDesc, url: `/${lang}/contact` },
  };
}
              ...
              <div className="lg:sticky lg:top-24">
                  ...
                  {contact.channels.map((c) => {
                    ...
                    return (
                      <div key={c.label}>
                        ...
                        <dd className="mt-1.5">
                          {href ? (
                            <a href={href} className="font-medium text-zinc-900 underline-offset-4 hover:underline dark:text-zinc-100">
                              {c.value}
                            </a>
                          ) : (
                            <span className="font-medium text-zinc-900 dark:text-zinc-100">{c.value}</span>
                          )}
                        </dd>
                ...
                <div className="mt-10">
                  <p className="text-xs font-semibold tracking-[0.2em] text-zinc-500 uppercase dark:text-zinc-400">
                    {contact.collabStartLabel}
                  </p>
                  <ol className="mt-3 space-y-3">
                    {contact.steps.map((s) => (
                      <li key={s.step} className="flex items-start gap-3">
                        <span className="text-gradient mt-0.5 text-sm font-bold tracking-tight">{s.step}</span>
                        <div>
                          <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{s.title}</p>
                          <p className="text-xs text-zinc-600 dark:text-zinc-400">{s.body}</p>
                        </div>
                      </li>
                    ))}
                  </ol>
