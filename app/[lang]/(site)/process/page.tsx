...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    title: dict.process.metaTitle,
    description: dict.process.metaDesc,
    alternates: { canonical: `/${lang}/process` },
    openGraph: { title: dict.process.metaTitle, description: dict.process.metaDesc, url: `/${lang}/process` },
  };
}
    ...
    <>
            ...
            {process.steps.map((s, i) => (
              ...
              >
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
                  <div>
                    <p className="text-gradient text-5xl font-bold tracking-tight">{s.step}</p>
      ...
      <section className="relative overflow-hidden border-b border-zinc-200/70 bg-white dark:border-zinc-800/70 dark:bg-black">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-24 sm:py-28">
          <Reveal>
            <p className="text-sm font-medium tracking-[0.2em] text-indigo-600 uppercase dark:text-indigo-400">
              {process.agreementsEyebrow}
            </p>
            <h2 className="mt-2 max-w-3xl text-3xl font-semibold tracking-tight text-zinc-900 sm:text-5xl dark:text-zinc-50">
              <span className="text-gradient">{process.agreementsTitleHighlight}</span>
            </h2>
            <p className="mt-4 max-w-2xl text-zinc-600 dark:text-zinc-300">
              {process.agreementsDesc}
            </p>
          </Reveal>
          <ul className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {process.agreements.map((a, i) => (
              <Reveal
                key={a.title}
                as="li"
                delay={i * 110}
